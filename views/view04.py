...
def main():
    ...
    def decorate_df_ctb(df: pd.DataFrame,
        ...
        if brand == "sleeper":
            ...
            required = ['날짜', '검색량', '기본 검색량', '기본 검색량_비중',
                        '태요미네', '태요미네_비중', '노홍철 유튜브', '노홍철 유튜브_비중', '아울디자인', '아울디자인_비중', '알쓸물치', '알쓸물치_비중', '홈스타일링연구소', '홈스타일링연구소_비중', '손태영', '손태영_비중']            
            for c in required:
                if c not in df.columns:
                    df[c] = 0
            ...
            df = df[['날짜', '검색량', '기본 검색량', '기본 검색량_비중',
                     '손태영', '손태영_비중', '홈스타일링연구소', '홈스타일링연구소_비중', '노홍철 유튜브', '노홍철 유튜브_비중', '태요미네', '태요미네_비중',  '아울디자인', '아울디자인_비중', '알쓸물치', '알쓸물치_비중']]
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["손태영", "홈스타일링연구소", "노홍철 유튜브", "태요미네", "베리엠제이", "아울디자인", "알쓸물치"])
    ...
    with tab1:
        ...
        # 데이터 전처리 1
        cols_to_int = ['손태영', '홈스타일링연구소', '태요미네', '노홍철 유튜브', '아울디자인', '알쓸물치', '검색량']
        ...
        # 신규컬럼 생성 - 기본 검색량
        df_QueryContribution["기본 검색량"] = df_QueryContribution["검색량"] - df_QueryContribution[['손태영', '홈스타일링연구소', '태요미네','노홍철 유튜브', '아울디자인', '알쓸물치']].sum(axis=1)
        # 신규컬럼 생성 - 비중
        cols = ['손태영', '홈스타일링연구소', '노홍철 유튜브', '태요미네', '아울디자인', '알쓸물치', '기본 검색량']
        for col in cols:
