...
#     """ 특정 컬럼(또는 여러 컬럼)에 background_gradient 적용

# 합계/평균 행은 색칠 제외 후, 마지막에 합계/평균 행 전체를 회색톤으로 하이라이트

# (현재 vmin/vmax를 “0 제외 최소값” 기반으로 잡고 0은 흰색 처리하는 규칙 포함)"""
...
#             # ───────────────────────────────
#             # (수정) min은 0이 아닌 값 중 최소값
#             nonzero_min = s_finite[s_finite != 0].min()
#             if pd.isna(nonzero_min):
#                 nonzero_min = s_finite.min()

#             vmin_raw = nonzero_min if vmin is None else vmin
#             vmax_raw = s_finite.max() if vmax is None else vmax
